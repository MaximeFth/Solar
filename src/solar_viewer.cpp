...
keyboard(int key, int scancode, int action, int mods)
{
    if (action == GLFW_PRESS || action == GLFW_REPEAT)
    {
        ...
        switch (key)
        {
            ...
            case GLFW_KEY_8:
            {
                if (dist_factor_-0.1f > 2.5 && dist_factor_-0.1f < 20.0) dist_factor_ -= 0.1f;
                break;
            }

            case GLFW_KEY_9:
            {
                if (dist_factor_+0.1 > 2.5 && dist_factor_+0.1 < 20.0) dist_factor_ += 0.1f;
                break;
            }
...
void Solar_viewer::update_body_positions() {
    ...
    earth_.pos_= mat4::rotate_y(earth_.angle_orbit_)*mat4::translate(earth_.distance_*vec3(1,0,0))*sun_.pos_;
    mercury_.pos_ = mat4::rotate_y(mercury_.angle_orbit_)*mat4::translate(mercury_.distance_*vec3(1,0,0))*sun_.pos_;
    venus_.pos_ = mat4::rotate_y(venus_.angle_orbit_)*mat4::translate(venus_.distance_*vec3(1,0,0))*sun_.pos_;
    mars_.pos_ = mat4::rotate_y(mars_.angle_orbit_)*mat4::translate(mars_.distance_*vec3(1,0,0))*sun_.pos_;
    moon_.pos_ = mat4::rotate_y(earth_.angle_orbit_)*mat4::translate(earth_.distance_*vec3(1,0,0))*mat4::rotate_y(moon_.angle_orbit_)*mat4::translate(moon_.distance_*vec3(1,0,0))*sun_.pos_;
...
void Solar_viewer::paint()
{
    ...
    if(!in_ship_){
		vec3 v(planet_to_look_at_->pos_[0], planet_to_look_at_->pos_[1], planet_to_look_at_->pos_[2]);
		vec4 eye = mat4::translate(v)*mat4::rotate_y(y_angle_)*mat4::rotate_x(x_angle_)*mat4::translate(dist_factor_*planet_to_look_at_->radius_*vec3(0,0,1))*vec4(0,0,0,1);
		...
		vec3 n = normalize(vec3(eye));
		float alpha = dot(n,vec3(0,1,0));
		billboard_x_angle_ = -90 + rad2deg(acos(alpha));
		if(n[1]>0) billboard_x_angle_ = 180 + billboard_x_angle_;

		vec3 n2=normalize (vec3(n[0], 0, n[2]));
		float theta = acos(dot(n2,vec3(0,0,1)));
		billboard_y_angle_ = rad2deg(theta);
		if(n2[0]<0) billboard_y_angle_= 360 - billboard_y_angle_;

		mat4 projection = mat4::perspective(fovy_, (float)width_/(float)height_, near_, far_);
		draw_scene(projection, view);
	}
    else{
		vec3 v(ship_.pos_[0],ship_.pos_[1],ship_.pos_[2]);
		vec4 eye = mat4::translate(v)*mat4::rotate_y(y_angle_)*mat4::rotate_y(ship_.angle_)*mat4::rotate_x(10)*mat4::translate(dist_factor_*ship_.radius_*vec3(0,0.6,-5))*vec4(0,0,0,1);
		vec4 center = ship_.pos_;
		vec4 up = mat4::rotate_y(y_angle_)*vec4(0,1,0,1);
		mat4    view = mat4::look_at(vec3(eye), vec3(center), vec3(up));
...
void Solar_viewer::draw_scene(mat4& _projection, mat4& _view)
{
    ...
    //render stars
    m_matrix = mat4::translate(stars_.distance_*vec3(1,0,0))*mat4::scale(stars_.radius_);
    mv_matrix = _view * m_matrix;
    mvp_matrix = _projection * mv_matrix;
    color_shader_.use();
  ...
  //render space ship
    m_matrix = mat4::rotate_y(ship_.angle_)*mat4::translate(ship_.pos_)*mat4::scale(ship_.radius_);
    mv_matrix = _view * m_matrix;
    mvp_matrix = _projection * mv_matrix;
    n_matrix = mat3(mv_matrix);
    phong_shader_.use();
  ...
  //render earth
    m_matrix = mat4::rotate_y(earth_.angle_orbit_)*mat4::translate(earth_.distance_*vec3(1,0,0))*mat4::rotate_y(earth_.angle_self_)*mat4::scale(earth_.radius_);
    mv_matrix = _view * m_matrix;
    mvp_matrix = _projection * mv_matrix;
    n_matrix = mat3(mv_matrix);
    earth_shader_.use();
    ...
    earth_shader_.set_uniform("t", sun_animation_time, true );
    ...
    //render mercury
    m_matrix = mat4::rotate_y(mercury_.angle_orbit_)*mat4::translate(mercury_.distance_*vec3(1,0,0))*mat4::rotate_y(mercury_.angle_self_)*mat4::scale(mercury_.radius_);
    mv_matrix = _view * m_matrix;
    mvp_matrix = _projection * mv_matrix;
    n_matrix = mat3(mv_matrix);
    phong_shader_.use();
  ...
  //render venus
    m_matrix = mat4::rotate_y(venus_.angle_orbit_)*mat4::translate(venus_.distance_*vec3(1,0,0))*mat4::rotate_y(venus_.angle_self_)*mat4::scale(venus_.radius_);
    mv_matrix = _view * m_matrix;
    mvp_matrix = _projection * mv_matrix;
    n_matrix = mat3(mv_matrix);
    phong_shader_.use();
  ...
  //render mars
    m_matrix = mat4::rotate_y(mars_.angle_orbit_)*mat4::translate(mars_.distance_*vec3(1,0,0))*mat4::rotate_y(mars_.angle_self_)*mat4::scale(mars_.radius_);
    mv_matrix = _view * m_matrix;
    mvp_matrix = _projection * mv_matrix;
    n_matrix = mat3(mv_matrix); // inverse and transpose
    phong_shader_.use();
    phong_shader_.set_uniform("modelview_projection_matrix", mvp_matrix);
    phong_shader_.set_uniform("modelview_matrix", mv_matrix);
    phong_shader_.set_uniform("normal_matrix", n_matrix);
  ...
  //render moon
    m_matrix = mat4::rotate_y(earth_.angle_orbit_)*mat4::translate(earth_.distance_*vec3(1,0,0))*mat4::rotate_y(moon_.angle_orbit_)*mat4::translate(moon_.distance_*vec3(1,0,0))*mat4::rotate_y(moon_.angle_self_)*mat4::scale(moon_.radius_);
    mv_matrix = _view * m_matrix;
    mvp_matrix = _projection * mv_matrix;
    n_matrix = mat3(mv_matrix);
    phong_shader_.use();
