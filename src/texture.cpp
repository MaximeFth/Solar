    ...
    **/
    ...
    for (int col = 0; col < width; ++col) {
        for (int row = 0; row < height; ++row) {
            ...
            img[(row * width + col) * 4 + 2] = 51; // B
            img[(row * width + col) * 4 + 3] = 255; // A
                
            float dist = sqrt(pow(row-450,2) + pow(col-450,2));
            float max_dist = 300;
   
            if (dist>150) img[(row * width + col) * 4 + 3] = 255-250*(dist/max_dist);  
            if (dist>300) img[(row * width + col) * 4 + 3] = 0;  
